...
export default function AdminLayout({ children }) {
  const { adminSession, logoutAdmin, refreshData, loading } = useCart();
        ...
        <div>
          ...
          <nav className="p-4 space-y-1.5">
            ...
            <Link
              href="/admin/produk"
              className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl text-xs font-bold transition-all ${
                pathname.includes('/admin/produk')
                  ? 'bg-slate-800 text-white shadow-md'
                  : 'hover:bg-slate-800 text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <Link
              href="/admin/kategori"
              className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl text-xs font-bold transition-all ${
                pathname.includes('/admin/kategori')
                  ? 'bg-slate-800 text-white shadow-md'
                  : 'hover:bg-slate-800 text-slate-400 hover:text-slate-200'
              }`}
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="md:hidden">
              ...
              >
                ...
                <option value="/admin/produk">Kelola Produk & Stok</option>
                <option value="/admin/kategori">Kelola Kategori</option>
